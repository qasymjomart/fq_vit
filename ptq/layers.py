...
class QConv2d(nn.Conv2d):
    ...
    def __init__(self, 
                 ...
                 quant=False,
                 calibrate=False, # whether to calibrate the quantization parameters
                 last_calibrate=False, # 
                 bit_type=BIT_TYPE_DICT['int8'], # bit type for quantization
...
class QAct(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.quant:
            return x # because activation is not quantized during inference
        # we quantize the activation during inference, so we need to quantize it here
        x = self.quantizer(x)
        return x

class QIntLayerNorm(nn.LayerNorm):
    """This class is a wrapper for nn.LayerNorm to support int quantization.
    Normalized shape is a single integer, which is the number of features in the input tensor.
    """
    ...
    def __init__(self,
        ...
        assert isinstance(normalized_shape, int), "normalized_shape must be an int for QIntLayerNorm"
        self.mode = 'ln'
    
    def get_MN(self, x):
        bit = 8
        N = torch.clamp(bit - 1 - torch.floor(torch.log2(x)), 0, 31) # torch.clamp here simply ensures that N is in the range [0, 31], which is the range of valid exponents for 32-bit floating point numbers
    ...
    def forward(self, x, in_quantizer=None, out_quantizer=None, in_scale_expand=1):
        if self.mode == 'ln':
            ...
            in_scale = in_quantizer.scale # scale of input to LN
            if in_scale_expand != 1:
                in_scale = in_scale.unsqueeze(-1).expand(
                    -1, in_scale_expand).T.reshape(-1) # expand in_scale to match the number of channels in x
            out_scale = out_quantizer.scale # calibrated scale of output of LN
            assert in_scale is not None and out_scale is not None, "in_scale and out_scale must be set for int mode"
            channel_nums = x.shape[-1] # last dimension is the number of channels
            in_scale = in_scale.reshape(1, 1, -1) # reshape in_scale to match the shape of x
            out_scale = out_scale.reshape(1, 1, -1) # reshape out_scale to match the shape of x
            x_q = (x / in_scale).round() # quantize x to int using in_scale
            in_scale1 = in_scale.min() # find the minimum scale in in_scale
            in_scale_mask = (in_scale / in_scale1).round() # find the ratio of in_scale to in_scale1 and round it to int
            
            x_q = x_q * in_scale_mask # scale x_q by in_scale_mask to match the scale of in_scale1, this is to bring all channels to the same scale
            
            mean_x_q = x_q.mean(dim=-1) * in_scale1 # compute the mean of x_q and scale it back to the original scale using in_scale1
            ...
            A_sign = A.sign()
            M, N = self.get_MN(A.abs()) # get the M and N for quantization of A
